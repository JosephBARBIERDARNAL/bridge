...
- `apps/bridge/src/` contains the React Native/TypeScript UI and platform-specific clients. Use `.web.ts(x)` files for browser-only implementations.
...
- `just dev` starts the gateway and mock browser preview.
- `just dev-real` runs the preview against a local Ollama instance.
- `just test` runs all Rust workspace tests and Bun tests.
- `just check` runs Rust compilation and formatting checks, TypeScript type-checking, and the web build.
...
Rust tests live beside implementation code in `#[cfg(test)]` modules and use `#[test]` or `#[tokio::test]`. TypeScript tests use Bun’s test runner and the `*.test.ts` naming pattern, for example `src/client/mock.test.ts`. Add focused regression tests for changed behavior. No coverage threshold is currently enforced.

## Security & Configuration

Copy `.env.example` for local settings, but never commit tokens, `.env` files, databases, keystores, or built APKs. Release APKs require a local `apps/bridge/android/keystore.properties`.
